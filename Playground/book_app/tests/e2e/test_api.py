...
class TestBooksAPI(unittest.TestCase):
    ...
    def test_get_all_books_with_category_filter(self):
        """Test GET /books?category=<category> filters results."""
        response = self.client.get("/books?category=Self-Help")
        self.assertEqual(response.status_code, 200)
        data = response.json()
    ...
    def test_get_all_books_category_not_found(self):
        """Test GET /books with non-existent category."""
        response = self.client.get("/books?category=nonexistent")
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.json()), 0)

    def test_get_book_by_title_success(self):
        """Test GET /books/{title} returns correct book."""
        response = self.client.get("/books/Atomic Habits")
        self.assertEqual(response.status_code, 200)
        data = response.json()
    ...
    def test_get_book_by_title_not_found(self):
        """Test GET /books/{title} returns 404 for non-existent book."""
        response = self.client.get("/books/Nonexistent Book Title")
        self.assertEqual(response.status_code, 404)
        self.assertIn("not found", response.json()["detail"])

    def test_get_book_by_id_success(self):
        """Test GET /books/{id}/details returns correct book."""
        response = self.client.get("/books/1/details")
        self.assertEqual(response.status_code, 200)
        data = response.json()
        self.assertEqual(data["id"], 1)
        self.assertEqual(data["title"], "Atomic Habits")
    ...
    def test_get_book_by_id_not_found(self):
        """Test GET /books/{id}/details returns 404 for non-existent ID."""
        response = self.client.get("/books/999/details")
        self.assertEqual(response.status_code, 404)
        self.assertIn("not found", response.json()["detail"])
    ...
    def test_create_book_title_too_short(self):
        """Test POST /books with title < 5 chars returns 422."""
        payload = {
            "title": "abc",
            "author": "Author",
            "category": "Test",
    ...
    def test_create_book_title_too_long(self):
        """Test POST /books with title > 100 chars returns 422."""
        payload = {
            "title": "x" * 101,
            "author": "Author",
            "category": "Test",
    ...
    def test_create_book_author_too_short(self):
        """Test POST /books with author < 2 chars returns 422."""
        payload = {
            "title": "Valid Title",
            "author": "A",
            "category": "Test",
    ...
    def test_create_book_category_too_short(self):
        """Test POST /books with category < 3 chars returns 422."""
        payload = {
            "title": "Valid Title",
            "author": "Author",
            "category": "ab",
    ...
    def test_create_book_missing_category(self):
        """Test POST /books with missing category returns 422."""
        payload = {
            "title": "Valid Title",
            "author": "Author",
        }
    ...
    def test_update_book_success(self):
        """Test PUT /books/{id} updates book."""
        payload = {
            "title": "Updated Title",
            "author": "Updated Author",
            "category": "Updated Category",
        }
        response = self.client.put("/books/1", json=payload)
        self.assertEqual(response.status_code, 204)

        get_response = self.client.get("/books/1/details")
    ...
    def test_update_book_partial(self):
        """Test PUT /books/{id} with partial update."""
    ...
    def test_update_book_not_found(self):
        ...
        response = self.client.put("/books/999", json=payload)
        self.assertEqual(response.status_code, 404)
        self.assertIn("not found", response.json()["detail"])

    def test_update_book_validation_error(self):
        """Test PUT /books/{id} with invalid data returns 422."""
        payload = {"title": "ab"}
        response = self.client.put("/books/1", json=payload)
        self.assertEqual(response.status_code, 422)

    def test_delete_book_success(self):
        """Test DELETE /books/{id} deletes book."""
    ...
    def test_route_ordering_details_before_title(self):
        """Test that /books/1/details works (not treated as title)."""
        response = self.client.get("/books/1/details")
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()["id"], 1)
